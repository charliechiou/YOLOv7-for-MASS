            ...
            with torch.no_grad():
                ...
                for cls, (x1, y1, x2, y2), conf in result[0][1]:
                    ...
                    if names[cls] == self.num_chose_control.currentText():
                        ...
                        if (x1+x2)/2 > 320:
                            self.degree = str(203.53*(((x1+x2)/2/640)-0.5)*(((x1+x2)/2/640)-0.5)+32.579*(((x1+x2)/2/640)-0.5)+0.0794)
                            self.direction = "Right"
                        if (x1+x2)/2 < 320:
                            self.degree = str(203.53*(0.5-((x1+x2)/2/640))*(0.5-((x1+x2)/2/640))+32.579*(0.5-((x1+x2)/2/640))+0.0794)
                            self.direction = "Left"
                    else:
                        self.x = "none"
                        self.y = "none"
