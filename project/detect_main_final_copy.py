...
class MainWindows(QFrame, detect_gui.Ui_main):
    ...
    sound_direction_signal = pyqtSignal(str)
    single_display_signal = pyqtSignal(str)
    up_display_signal = pyqtSignal(str)
    down_display_signal = pyqtSignal(str)
    ...
    def __init__(self):
        ...
        self.audio_detect = threading.Thread(target=self.audio)
        self.audio_detect.start()
        self.audio_start = False
        self.soundDirection = 'none'
        self.sound_direction_signal.connect(self.sound_direction_display)
        self.single_display_signal.connect(self.single_direction_display)
        self.up_display_signal.connect(self.up_direction_display)
        self.down_display_signal.connect(self.down_direction_display)
    ...
    ## audio detect ##
    def sound_direction_display(self,dir):
        self.result_display.setText(dir)

    def single_direction_display(self,single):
        self.single_display.setText(single)

    def up_direction_display(self,up):
        self.up_display.setText(up)

    def down_direction_display(self,down):
        self.down_display.setText(down)
    ...
    def audio(self):
        ...
        while True:
            ...
            if time.time() - self.start_time > 60 and self.audio_done == False:
                ...
                if lines is not None:
                    ...
                    self.sound_direction_signal.emit(self.soundDirection)
                    self.single_display_signal.emit(str(slope_0_count))
                    self.up_display_signal.emit(str(slope_1_count))
                    self.down_display_signal.emit(str(slope_minus_1_count))
